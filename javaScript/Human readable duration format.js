//URL--
https://www.codewars.com/kata/52742f58faf5485cae000b9a

//INSTRUCTIONS--
/* 
Your task in order to complete this Kata is to write a function which formats a duration, given as a number of seconds, in a human-friendly way.

The function must accept a non-negative integer. If it is zero, it just returns "now". Otherwise, the duration is expressed as a combination of years, days, hours, minutes and seconds.

It is much easier to understand with an example:

* For seconds = 62, your function should return 
    "1 minute and 2 seconds"
* For seconds = 3662, your function should return
    "1 hour, 1 minute and 2 seconds"

For the purpose of this Kata, a year is 365 days and a day is 24 hours.

Note that spaces are important.
Detailed rules

The resulting expression is made of components like 4 seconds, 1 year, etc. In general, a positive integer and one of the valid units of time, separated by a space. The unit of time is used in plural if the integer is greater than 1.

The components are separated by a comma and a space (", "). Except the last component, which is separated by " and ", just like it would be written in English.

A more significant units of time will occur before than a least significant one. Therefore, 1 second and 1 year is not correct, but 1 year and 1 second is.

Different components have different unit of times. So there is not repeated units like in 5 seconds and 1 second.

A component will not appear at all if its value happens to be zero. Hence, 1 minute and 0 seconds is not valid, but it should be just 1 minute.

A unit of time must be used "as much as possible". It means that the function should not return 61 seconds, but 1 minute and 1 second instead. Formally, the duration specified by of a component must not be greater than any valid more significant unit of time.



*/

//SOLUTION--

function formatDuration (seconds) {

   let retArr = []
   
  if(seconds === 0){
    return "now"
  }
  
  if(seconds < 60){
    let s = seconds;
    let sStr =  (s === 1) ? `${s} second` : `${s} seconds`
    retArr.push(sStr)
  }

  if(seconds >= 60 && seconds < 3600) {
    let m = Math.floor(seconds/60)
    let mStr = (m === 1) ? `${m} minute` : `${m} minutes`
    retArr.push(mStr)
    let s = seconds%60;
    let sStr =  (s === 1) ? `${s} second` : `${s} seconds`
    retArr.push(sStr)
    
  }

if(seconds >= 3600 && seconds < 86400) {
    let h = Math.floor(seconds/3600)
    let hStr = (h === 1) ? `${h} hour` : `${h} hours`
    retArr.push(hStr)
    let m =  Math.floor((seconds%3600) / 60)
    let mStr = (m === 1) ? `${m} minute` : `${m} minutes`
    retArr.push(mStr)
    let s = seconds - ((h*3600) + (m*60))
    let sStr =  (s === 1) ? `${s} second` : `${s} seconds`
    retArr.push(sStr)
  }


if(seconds >= 86400 && seconds < 31536000) {
    let d = Math.floor(seconds/86400)
    let dStr = (d === 1) ? `${d} day` : `${d} days`
    retArr.push(dStr)
    let h = Math.floor((seconds%86400) / 3600)
    let hStr = (h === 1) ? `${h} hour` : `${h} hours`
    retArr.push(hStr)
    let m =  Math.floor( (seconds - ((d*86400) + (h*3600))) / 60)
    let mStr = (m === 1) ? `${m} minute` : `${m} minutes`
    retArr.push(mStr)
    let s = seconds - ((d*86400) + (h*3600) + (m*60))
    let sStr =  (s === 1) ? `${s} second` : `${s} seconds`
    retArr.push(sStr)
  }


  if(seconds >= 31536000) {
    let y = Math.floor(seconds/31536000)
    let yStr = (y === 1) ? `${y} year` : `${y} years`
    retArr.push(yStr)
    let d = Math.floor((seconds%31536000) / 86400)
    let dStr = (d === 1) ? `${d} day` : `${d} days`
    retArr.push(dStr)
    let h = Math.floor( (seconds - ((y*31536000) + (d*86400))) / 3600)
    let hStr = (h === 1) ? `${h} hour` : `${h} hours`
     retArr.push(hStr)
    let m =  Math.floor( (seconds - ((y*31536000) + (d*86400) + + (h*3600))) / 60)
    let mStr = (m === 1) ? `${m} minute` : `${m} minutes`
     retArr.push(mStr)
    let s = seconds - ((y*31536000)+ (d*86400) + (h*3600) + (m*60))
    let sStr =  (s === 1) ? `${s} second` : `${s} seconds`
     retArr.push(sStr)
  }

console.log(retArr)
retArr = retArr.filter( (item,index) => item.charAt(0) > 0)  
console.log(retArr)

let x = retArr.length;
switch (x) {
  case 1:
    return retArr[0]
    break;
  case 2:
    return `${retArr[0]} and ${retArr[1]}`
    break;
  case 3:
    return `${retArr[0]}, ${retArr[1]} and ${retArr[2]}`
    break; 
  case 4:
     return `${retArr[0]}, ${retArr[1]}, ${retArr[2]} and ${retArr[3]}`
    break;
  case 5:
     return `${retArr[0]}, ${retArr[1]}, ${retArr[2]}, ${retArr[3]} and ${retArr[4]}`
    break;
  
}
  
}


//TEST_CASES

describe("Tests", () => {
  it("test", () => {
Test.assertEquals(formatDuration(1), "1 second");
Test.assertEquals(formatDuration(62), "1 minute and 2 seconds");
Test.assertEquals(formatDuration(120), "2 minutes");
Test.assertEquals(formatDuration(3600), "1 hour");
Test.assertEquals(formatDuration(3662), "1 hour, 1 minute and 2 seconds");

  });
});

